...
=begin
  # Does not work because strings are converted to booleans before this point.
  def handle_binary_questions
    Questionnaire.columns_hash.each do |k,v|
      if v.type == :boolean
        if self[k.to_s].downcase == 'y'
          self[k.to_s] = true
        elsif self[k.to_s].downcase == 'n'
          self[k.to_s] = false
        else
          self[k.to_s] = nil
        end
